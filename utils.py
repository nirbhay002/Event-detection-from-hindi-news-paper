...
def get_entity_info(bio_labels, tokens, text=None, spans=None):
    """
    Extracts entity spans and metadata from BIO-tagged tokens.

    This function processes a list of BIO-formatted labels and their corresponding tokens
    to identify and group entity spans. For each entity, it returns a dictionary containing:
    - the entity label (e.g., "PLACE-ARG", "NATURAL_EVENT.FLOODS"),
    - the list of tokens that make up the entity,
    - the start and end indices in the token list,
    - the phrase of the entity (as a joined string).

    If original text and character-level span information are provided,
    the function can also be extended to compute character positions, though
    this is optional and not used in the current implementation.

    Parameters:
        bio_labels (List[str]): A list of BIO-formatted labels (e.g., "B-PLACE-ARG", "I-...", "O").
        tokens (List[str]): A list of tokens corresponding to the labels.
        text (str, optional): The original raw text (not used unless spans are provided).
        spans (List[Tuple[int, int]], optional): Character-level spans for each token as (start, end).

    Returns:
        List[Dict]: A list of dictionaries, each representing an entity with keys:
            - 'tokens': List of tokens in the entity span.
            - 'label' : The entity label (without BIO prefix).
            - 'start' : Start index in the token list.
            - 'end'   : End index (exclusive) in the token list.
            - 'entity_phrase': String representation of the entity.
    """
